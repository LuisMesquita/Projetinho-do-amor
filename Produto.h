...
int getIndiceProduto(char codigo[10], Produto *repositorioProduto){ // RETORNA O INDICE DO PRODUTO COM O C�DIGO PESQUISADO.
	int indice;
...
ProductResponse procurarProduto(char codigo[10], Produto *repositorioProduto){ // RETORNA O PRODUTO DO C�DIGO PESQUISADO.
	...
	if(strcmp(codigo, repositorioProduto[indice].codigo) == 0){
		...
		response.error = 1; // TRATAR AS MENSAGENS DE EROO NO MAIN /  PRODUTO N�O ENCONTRADO
	}
	return response;
}
